...
class Pack {
    ...
    public function countAvailablePacks()
    {
        $query = "SELECT COUNT(*) as total FROM pack WHERE status = 'available'";
        $stmt = $this->pdo->prepare($query); // ❗ Assure-toi que $this->conn est bien défini
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        return $result['total'];
    ...
    public function countUnavailablePacks()
    {
        $query = "SELECT COUNT(*) as total FROM pack WHERE status = 'unavailable'";
